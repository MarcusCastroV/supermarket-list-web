...
export const Modal = ({ onClose, item }) => {
    ...
    const callUpdateItem = async () => {
         ...
         const validade = validadeBeforeSave();
        if (validade) {
            const result = await updateItem( item?._id,{ name, quantity: Number(quantity), checked: item?.checked })
            if (!result?.error) {
    ...
    return (
        <div className="modal">
            <div className="modal-content">
                ...
                <Input onChange={(text) => setName(text)} value={name} label="Nome" placeholder="Ex: Arroz" />
                <Input onChange={(text) => setQuantity(text)} value={quantity} label="Quantidade" type="number" />
                <div className='buttons-container'>
                
                <Button onClick={item ? callUpdateItem : callAddItem}>{item ? 'Atualizar' : 'Adicionar'}</Button>
                {
                    item && (
